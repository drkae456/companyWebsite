...
def perform_search(prompt: str, limit: int = 5) -> list[Any]:
    """
    ...
    Args:
        prompt: The query or keywords to search for
        limit: Maximum number of results to return
        
    Returns:
        List of formatted search results or empty list if no results
    """
    ...
    try:
        ...
        if not prompt or len(prompt.strip()) < 2:
            logger.warning("Search prompt is empty or too short")
            return []
            
        # Initialize search engine models
        models_map = initialize_search_engine()
        if not models_map:
            logger.error("Failed to initialize search engine models")
            raise ValueError("Search engine initialization failed - no models available")
            
        # Identify model from prompt
        model = identify_model_from_prompt(prompt, models_map)
        
        if not model:
            logger.warning("No suitable model found for search")
            # Try to use a default model instead of failing
            default_models = ['BlogPost', 'Webpage', 'Project']
            for default_model_name in default_models:
                if default_model_name in models_map:
                    model = models_map[default_model_name]
                    logger.info(f"Using default model: {model.__name__}")
                    break
            
            if not model:
                logger.error("No suitable model or default model found for search")
                return []
        ...
        if isinstance(prompt, list):
            # If prompt is already a list of keywords
            keywords = prompt
        else:
        ...
        if len(keywords) == 1 and keywords[0].lower() in model_variations:
            logger.info("Model-only query detected - returning most recent items")
            ...
            try:
                results = list(model.objects.all().order_by(order_field)[:limit])
            except Exception as e:
                logger.error(f"Database query error: {str(e)}", exc_info=True)
                raise ValueError(f"Database error querying {model.__name__}: {str(e)}")
        ...
        try:
            ...
            project_patterns = ['appattack', 'malware', 'pt gui', 'smishing', 'vr', 'cybersafe', 'threatmirror']
            is_project_search = any(pattern in (prompt.lower() if isinstance(prompt, str) else 
                                  ' '.join(keywords).lower()) for pattern in project_patterns)
            ...
            # Execute search
            results = list(model.objects.filter(q_objects).distinct()[:limit])
            ...
            for result in results:
                try:
                    ...
                    # Add a simplified result instead of failing completely
                    formatted_results.append({
                        'model': model.__name__.lower(),
                        'id': getattr(result, 'id', 'unknown'),
                        'error': 'Could not format result properly'
                    })
            ...
            raise ValueError(f"Database error during search: {str(query_error)}")
            
    except Exception as e:
        logger.error(f"Search error: {str(e)}", exc_info=True)
        # Re-raise with more context for the chatbot to provide better error message
        raise ValueError(f"Search engine error: {str(e)}")
